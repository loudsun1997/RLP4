...
from dqn_model import SplitQNetwork, DuelingDQNNoisy, SplitQNetworkNoisy
...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        # Define the reduced action space: up, down, left, right
        self.reduced_action_space = [0, 1, 2, 3]  # Map to up, down, left, right
        
        # Map reduced actions to ALE actions
        self.action_mapping = {
            0: 1,  # 'up' in ALE
            1: 2,  # 'right' in ALE
            2: 3,  # 'left' in ALE
        ...
        self.max_memory = int(5e4) # Memory size for replay buffer
        self.num_episodes = int(10e4) # Total episodes for training
        ...
        if args.buffer_type == 'std_buff':
            ...
            self.buffer = PrioritizedReplayBuffer(capacity=self.max_memory, alpha=0.4)
            self.buffer_type = 'prioritized_buff'
        # Epsilon-greedy parameters
        self.epsilon = 1.0
        self.epsilon_min = 0.01 # Lower minimum epsilon to allow more exploration in complex tasks 
        self.epsilon_decay = (self.epsilon - self.epsilon_min) / self.decay_rate
        
        # Beta scheduling for PRB
        self.beta = 0.4
        self.beta_increment = (1.0 - self.beta) / self.num_episodes
        ...
        elif args.dqn_type == 'split dqn':
            self.policy_net = SplitQNetwork(input_dim=4, output_dim=env.action_space.n).to(self.device)
            self.target_net = SplitQNetwork(input_dim=4, output_dim=env.action_space.n).to(self.device)
        elif args.dqn_type == 'split dqn noisy':
            self.policy_net = SplitQNetworkNoisy(input_dim=4, output_dim=env.action_space.n).to(self.device)
            self.target_net = SplitQNetworkNoisy(input_dim=4, output_dim=env.action_space.n).to(self.device)
        elif args.dqn_type == 'ddqn noisy':
            self.policy_net = DuelingDQNNoisy(input_dim=4, output_dim=env.action_space.n).to(self.device)
            self.target_net = DuelingDQNNoisy(input_dim=4, output_dim=env.action_space.n).to(self.device)
        ...
        if args.test_dqn:
            ...
            ###########################
            # YOUR IMPLEMENTATION HERE #
            self.policy_net.load_state_dict(torch.load('logs/905802/model.pth', map_location=self.device))
            self.target_net.load_state_dict(self.policy_net.state_dict())
    ...
    def make_action(self, observation, test=False):
        ...
        # Create a mapping from reduced actions to their corresponding Q-values
        valid_action_indices = list(self.action_mapping.values()) # Indices for UP, RIGHT, LEFT, DOWN in the original action space
        reduced_q_values = q_values[0][valid_action_indices]  # Extract Q-values for valid actions

        # Select action based on the epsilon-greedy policy
        if random.random() > self.epsilon:
            reduced_action = np.argmax(reduced_q_values) # Choose action with highest Q-value
            # print(f"Q-values: {q_values}, Reduced action: {reduced_action}")
        else:
            reduced_action =  random.randint(0, 3)  # Random action 
            # random.randint(0, self.env.action_space.n - 1) # Random action for exploration
        # Validate reduced_action to avoid KeyError
        if reduced_action not in self.action_mapping:
            raise ValueError(f"Invalid action generated: {reduced_action}")
        # Map reduced action to ALE action
        mapped_action = self.action_mapping[reduced_action]
        ###########################
    ...
    def update(self):
        ...
        if self.dqn_type == 'dqn' or self.dqn_type == 'split dqn' or self.dqn_type == 'split dqn noisy':
            with torch.no_grad():
                next_actions = self.target_net(next_states).argmax(1).unsqueeze(1)
                next_q_values = self.target_net(next_states).gather(1, next_actions)
                target_q_values = rewards + (self.gamma * next_q_values * (1 - dones))
        elif self.dqn_type == 'double dqn' or self.dqn_type == 'double dueling dqn' or self.dqn_type == 'ddqn noisy':
            with torch.no_grad():
